...
const DC = MC.Clip;
const TimeCapsule = MC.TimeCapsule;
...
/*************************** PLUGIN DEFINITION *************************/
/**********************************************************************/
class MyPluginIncident extends MC.API.Incident {
    onGetContext() {
...
test('Test single execution with steps of 100ms', () => {
    ...
    loggedValues = [];
    for (let i = 0; i < 25; i++) {
        myClip.realClip.step(i * 100, true);
...
test('Test execution of Clip after completion', () => {
    // step 1 / emulated play() command
    myClip.play(true);
    loggedValues = [];
    clipsStates = [];
...
test('Test pause / resume functionality', done => {
    ...
    clipsStates = [];
    for (let i = 0; i < 5; i++) {
        myClip.realClip.step(i * 300, true);
        clipsStates.push(myClip.runTimeInfo.state);
    }
    myClip.pause();
    ...
    setTimeout(() => {
        myClip.play(true);
        for (let i = 5; i < 8; i++) {
            myClip.realClip.step(i * 300 + delay, true);
            clipsStates.push(myClip.runTimeInfo.state);
        }
        expect(clipsStates).toEqual(
            ['playing', 'playing', 'playing', 'playing', 'playing',
...
test('Test time journey on pause state', done => {
    // step 1 / emulated play() command
    myClip.play(true);
    loggedValues = [];
    clipsStates = [];

    myClip.realClip.step(1500, true);
    clipsStates.push(myClip.runTimeInfo.state);

    // paused on ms: 1500
    myClip.pause();
    const journey = TC.startJourney(myClip);
    journey.station(1200);
    journey.destination();
